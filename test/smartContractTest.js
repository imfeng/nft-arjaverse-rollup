...
// describe("Arja Minting", function () {
...
//     arjaNFT = await arjaNFTFactory.deploy()
...
//       privateAirdrop: privateAirdrop.address,
//       plonk: plonk.address,
//     })
...
//     const quantity = 7;
//     const tx = await arjaNFT.mintRoleToAirdrop(daoName, daoRole, quantity, privateAirdrop.address);
//     tx.wait();
//     assert.equal(await arjaNFT.balanceOf(privateAirdrop.address), 7);
...
//     await expect(arjaNFT.connect(notOwnerSigner).mintRoleToAirdrop(daoName, daoRole, quantity, privateAirdrop.address)).to.be.reverted;
//   });
...
describe("Arja Airdrop", function () {
  ...
  before(async function () {
    ...
    const arjaNFTFactory = await hre.ethers.getContractFactory("ArjaGenerativeNFT")
    arjaNFT = await arjaNFTFactory.deploy()
    console.log("Contract arjaNFT deployed @", arjaNFT.address)
    ...
    // NFT MINTING
    let daoName = "Arja Badges"
    let daoRole = "Hardcore contributor"
    let quantity = "8";
    await arjaNFT.mintRoleToAirdrop(daoName, daoRole, quantity, privateAirdrop.address);
  ...
  it("should allow a user that provides a valid proof to collect an NFT from the airdrop contract", async function () {
    ...
    let balance = await arjaNFT.balanceOf(collector.address)
  ...
  it("should expect that the airdropped token is not transferable", async function () {
    ...
    await expect(arjaNFT.connect(collector).transferFrom(collector.address, receiver.address, 1)).to.be.reverted
  });
